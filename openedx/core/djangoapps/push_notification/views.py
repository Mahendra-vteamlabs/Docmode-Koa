...
from openedx.core.djangoapps.push_notification.tasks import send_new_message_notification
...
class TokenAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if mobile_device_token and mobile_device_id and mobile_version and mobile_device_type:
    ...
    def post(self,request):
        try:
            ...
            if username:
                ...
                notifications = DocmodeNotification.objects.filter(users__username=username)
                data = []
                for notification in notifications:
                    ...
                    if course_overview:
                        ...
                        notification_detail['date'] = notification.scheduled_time.strftime(
                        "%Y-%m-%dT%H:%M:%S.%fZ")
                        notification_detail['course_id'] = str(notification.course_id)
                        notification_detail['notification_type'] = notification.redirection_type
        ...
        try:
            if request.GET.get('user_id'):
                try:
                    ...
                    return JsonResponse({'status': 404, 'message': "User Does Not Exits."})
                access_obj = AccessToken.objects.filter(user=user)
                if access_obj.exists():
                    if pytz.utc.localize(datetime.now()) <= access_obj.order_by('-id')[:1].get().expires:
                        access_obj = AccessToken.objects.filter(user=user).order_by(
                            '-id')[1:].values_list("id", flat=True)
                        AccessToken.objects.filter(
                            pk__in=list(access_obj)).delete()
                        log.info("Successfully Login for User: {}".format(
                            str(user.username)))
                        return JsonResponse({'status': 200, 'message': "Successfully Login."})
                    else:
                        log.info("Your Access Token has been expired for User: {}".format(
                            str(user.username)))
                        return JsonResponse({'status': 202, 'message': "Your Access Token has been expired."})
                else:
                    log.info("Successfully Login for User: {}".format(
                        str(user.username)))
                    return JsonResponse({'status': 200, 'message': "Successfully Login."})
            ...
            return JsonResponse({'status': 404, 'message': "Error while CustomAuthentications, Please try again."})
