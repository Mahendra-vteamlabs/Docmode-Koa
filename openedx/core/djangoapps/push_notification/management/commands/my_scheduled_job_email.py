...
log = logging.getLogger("my_schedule_job_email")
...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        log.info("email cron job called")
        ...
        for course in courses:
            ...
            course_create_date = course_create.date()
            course_start = course_start.replace(tzinfo=None)
            notification_start_datetime = course_start - relativedelta(seconds=1)
            notification_start_date = (
                notification_start_datetime.date() - relativedelta(hours=3)
            )
            ...
            if (
                current_date == notification_start_date
                and current_datetime <= notification_start_datetime
            ):
                sent_notification_course = DocmodeNotificationEmail.objects.filter(
        ...
        log.info("get all course list of email notification send")
        values = []
        value = {}
        ...
        values.append(value)
        log.info("send email notification task for %s", values)
        send_new_message_notification_email.delay(values=values)
