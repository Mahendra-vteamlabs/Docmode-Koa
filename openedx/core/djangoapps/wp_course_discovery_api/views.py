...
class CourseSearchView(DeveloperErrorViewMixin, APIView):
    ...
    def get(self, request, **kwargs):
        ...
        if results.get('total'):
            ...
            course_overviews = CourseOverview.objects.filter(id__in=course_ids).order_by('-start')
            starting = (page)*20
            ending = (page+1)*(20)
            ...
            for course_overview in course_overviews:
                ...
                course_enroll = CourseEnrollment.objects.filter(course_id=course_key, user_id=user_id)
                ...
                if course_enroll.exists():
                    ...
                    if course_extra_info.course_type == '2':
                        video_blocks_in_course = modulestore().get_items(course_key, qualifiers={'category': 'video'})
                        block_id = modulestore().get_items(course_key, qualifiers={'category': 'video'})
                        if block_id:
                            data = block_id[0].__dict__
                            if 'scope_ids' in data:
                                search_course['seq_block_id'] = data.get('scope_ids').def_id.to_deprecated_string()
                            else:
                        ...
                        if video_blocks_in_course:
                            ...
                            video_id = video_blocks_in_course[0].youtube_id_1_25 or video_blocks_in_course[0].youtube_id_0_75  or video_blocks_in_course[0].youtube_id_1_0 or video_blocks_in_course[0].youtube_id_1_25 or video_blocks_in_course[0].youtube_id_1_5
                            if video_id :
                                search_course['video_url'] = 'https://www.youtube.com/watch?v=' + video_id
                            else:
            ...
            for course_overview in course_overviews:
                ...
                if course_enroll.exists():
                    ...
                    if block_id:
                        data = block_id[0].__dict__
                        if 'scope_ids' in data:
                            seq_block_id = data.get('scope_ids').def_id.to_deprecated_string()
                        else:
                            seq_block_id = ''
                    else:
                        seq_block_id = ''
                    ...
                    if course_extra_info.course_type == '2':
                        video_blocks_in_course = modulestore().get_items(course_key, qualifiers={'category': 'video'})
                        if video_blocks_in_course:
                            video_id = video_blocks_in_course[0].youtube_id_1_25 or video_blocks_in_course[0].youtube_id_0_75  or video_blocks_in_course[0].youtube_id_1_0
                            if video_id :
...
class CourseCheckcourseWebinarView(DeveloperErrorViewMixin, APIView):
    ...
    def get(self,request,**kwargs):
        ...
        if qnadetails:
            data = {}
            if userstate:
                data = {'city':userstate,'show_webinar':"true",'feedback_url':feedback_url}
                message = ''
                result['Data'] = data
                result['Status'] = "true"
                result['Message'] = message
            ...
            message = {
                            "error": _('Course not found in webinar')
                        }
            data = {}
            if userstate:
                data = {'city':userstate,'show_webinar':"false",'feedback_url':feedback_url}
                message = ''
                result['Data'] = data
                result['Status'] = "true"
                result['Message'] = message
    ...
    def post(self,request):
        ...
        try:
            ...
            if result.status_code== 200:
                ...
                message = {
                            "error": _('google form not submit something happen')
                        }
                return JsonResponse({'status': 'false', 'message': message}, status=400)
        except:
            message = {
    ...
    def get(self, request, **kwargs):
        ...
        if results.get('total'):
            ...
            if 'org' not in request.GET and 'subjects' not in request.GET:
                ...
                course_data = CourseOverview.objects.filter(pk__in=cid,catalog_visibility="both").values('org').annotate(ccount=Count('org'))

                for course in course_data:
                    if course['org'] not in associations_list:
                        if course['ccount'] > 0:
                            results['facets']['org']['terms'].update({course['org']:course['ccount']})

            
                if request.GET.get('coursetype') == 'Lectures':
                    cat_course_count = course_extrainfo.objects.filter(course_type=2).exclude(category='').values('category').annotate(ccount=Count('category')).filter(ccount__gte=1).order_by('-ccount')
                else:
                    cat_course_count = course_extrainfo.objects.filter(course_type=1).exclude(category='').values('category').annotate(ccount=Count('category')).filter(ccount__gte=1).order_by('-ccount')
                categoryid = []
                for catid in cat_course_count:
                    categoryid.append(catid['category'])
                categories_list = categories.objects.filter(pk__in=categoryid).values()
                ...
                if categories_list:
                    for category in  categories_list:
                        ...
                        if category['topic_name'] not in topics_list:
                            ...
                            results['facets']['subjects']['terms'].update({category['topic_name']:count})
            ...
            if page > 1:
                pages = page - 1
                previous = settings.LMS_ROOT_URL + "/api/v1/course/search/discovery_local/?search_key={search_key}&user_id={user_id}&page={page}".format(
                    search_key=search_key, user_id=user_id, page=pages)
            else:
                previous = ""
            if results.get('total') > page * 20:
                results['pagination'] = {
                    ...
                    "previous": previous,
                    "num_pages": math.ceil(results.get('total') / float(size)),
                    "next": settings.LMS_ROOT_URL + "/api/v1/course/search/discovery_local/?search_key={search_key}&user_id={user_id}&page={page}".format(search_key=search_key, user_id=user_id, page=page)
                }
            else:
                results['pagination'] = {
                    "count": results.get('total'),
