...
def send_welcome_email(sender, event=None, user=None, course_id=None, **kwargs):
    ...
    if event == EnrollStatusChange.enroll:
        log.info(u'event->')
        try:
            course_email = Custom_email.objects.get(course_id=course_id,template_name='welcome_email.template')
        
            if course_email:
                course_extra_data = course_extrainfo.objects.get(course_id=course_email.course_id)
                ...
                if course_extra_data.google_calendar_url and diffdate.days > 1:
                    log.info(u'diffdate %s',diffdate)
                    email_context['google_calendar_url'] = course_extra_data.google_calendar_url
                ...
                email_msg.send()
                mail_to_users = custom_mail_to_users(course_emailid=course_email.id,user_id=user.id,user_email=email,course_id=course_id)
                mail_to_users.save()
            ...
            if course_extra_data.google_calendar_url and minutes[0] > 400:
            ...
            subject = 'Congratulations you are enrolled in ' + email_context['course_title']
            email_template = course_email.get_template()
            plaintext_msg = email_template.render_plaintext(course_email.text_message, email_context)
            html_msg = email_template.render_htmltext(course_email.html_message, email_context)
...
def send_course_completion_mail(sender, user, course_id, **kwargs): 
    try:
        course_email = Custom_email.objects.get(course_id=course_id,template_name='completion_mail.template')
