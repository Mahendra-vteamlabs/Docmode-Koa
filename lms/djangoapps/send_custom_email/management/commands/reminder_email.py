...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        non_passed_users = PersistentCourseGrade.objects.filter(
            user_id__in=enroll_userid, course_id=cid, letter_grade=""
        )
        nonpassed_userids = []
        ...
        for user in user_lastlogin:
            if user_lastlogin is not None:
                enrolldate = CourseEnrollment.objects.get(
                ...
                if diffdate.days >= 3:
                    try:
                        course_email = Custom_email.objects.get(
                            course_id=cid, template_name="reminder_email.template"
                        )
                        if course_email:
                            ...
                            email_context["login_url"] = (
                                "http://cims.docmode.org/login?next=" + cid
                            )
                            subject = (
                                "Keep learning in " + email_context["course_title"]
                            )
