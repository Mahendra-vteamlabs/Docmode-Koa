...
from lms.djangoapps.reg_form.models import extrafields, third_party_user_registration_log
...
from common.djangoapps.organizations.models import Organization, OrganizationMembers, SponsoringCompany, OrganizationSlider
...
from lms.djangoapps.course_extrainfo.views import category_courses_count,category_lectures_count,category_casestudies_count,course_ctype_number
#import for course_details api

from lms.djangoapps.course_api.forms import CourseDetailGetForm, CourseListGetForm
from lms.djangoapps.associations.serializers import CourseDetailSerializer, CourseSerializer
from lms.djangoapps.course_api.permissions import can_view_courses_for_username

#import for user_profile_api
from lms.djangoapps.userprofile_extrainfo.models import education, awards, research_papers, media_featured, clinic_hospital_address
...
def list_lectures(request):
    ...
    course_data = CourseOverview.objects.all().filter(pk__in=cid,catalog_visibility="both").order_by('-start')[:130]
...
def association_about(request, organization_id):
    ...
    try:
        ...
        if data :
            ...
            try:
                courses = CourseOverview.objects.all().filter(display_org_with_default=organization_id,catalog_visibility="both").order_by('start')[::-1]
            except ObjectDoesNotExist:
                courses = None

            try:
                uc_courses = CourseOverview.objects.all().filter(Q(display_org_with_default=organization_id) & Q(start__gte=datetime.date.today()) & Q(catalog_visibility="both")).order_by('start')
            except ObjectDoesNotExist:
                uc_courses = None
...
def category(request, category_id):
    ...
    course_data = CourseOverview.objects.all().filter(pk__in=cid,catalog_visibility='both').order_by('start')[::-1]
    ...
    if request.is_ajax():
      if request.method == 'POST' :  
        ...
        if subcatid > '0':
          try:
            subcat_courses = course_extrainfo.objects.filter(category=cat.id,sub_category__contains=subcatid).values()
          except ObjectDoesNotExist:
            subcat_courses = None
...
def export_specz_usercount_csv(request):
    ...
    specQset = extrafields.objects.filter(user_type='dr').exclude(specialization_id__isnull=True).values('specialization_id').annotate(dcount=Count('specialization_id')).order_by('-dcount')
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename=Specialization_usercount_'+str(datetime.date.today())+'.csv'
...
def export_order_csv(request):
    ...
    db = MySQLdb.connect("hawthorn-live.c2woekolusus.ap-south-1.rds.amazonaws.com","ecomm001","HjDzeQZlRJcXoRadc8fRnS8HDnlky450mhL","ecommerce")
    cursor = db.cursor()
    cursor.execute("SELECT order_order.number,order_order.status,order_order.user_id,order_order.total_incl_tax,order_order.date_placed,ecommerce_user.id,ecommerce_user.email from order_order INNER JOIN ecommerce_user on ecommerce_user.id=order_order.user_id where order_order.status='complete' and ecommerce_user.email not like'%docmode%'")
    results = cursor.fetchall()
    db.close()

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename=Docmode_orders_'+str(datetime.date.today())+'.csv'
    writer = csv.writer(response, csv.excel)
    response.write(u'\ufeff'.encode('utf8')) # BOM (optional...Excel needs it to open UTF-8 file properly)
    #log.info(u" coupondata %s", coupon_results)
    user = request.user
...
def association_course_analytics(request,course_id):
    ...
    user_country_count = extrafields.objects.filter(user__id__in=cenrolls).exclude(rpincode=0).values('rcountry').annotate(dcount=Count('rcountry')).distinct()
...
def export_csv(request, course_id, datatype):
    ...
    if (user.is_staff):
        ...
        if datatype == 'enrolled' :
            ...
            vrows = StudentModule.objects.filter(course_id=cid,module_type='video').values('student_id','created','state').annotate(dcount=Count('student_id'))[:7000]
            for vrow in vrows:
        ...
        if datatype == 'enrolled' :
            ...
            response['Content-Disposition'] = 'attachment; filename='+ str(course_number[1]) +'_viewers.csv'
            vrows = StudentModule.objects.filter(course_id=cid,module_type='video')[:7000]
            for vrow in vrows:
                assoc_user = User.objects.get(id=vrow.student_id)
                user_ext = userdetails(assoc_user.id)
                writer.writerow([
...
def search_list(request):
    if request.is_ajax():
        if request.method == 'GET' :
            ...
            results = CourseOverview.objects.filter(Q(display_name__icontains=search_term)|Q(display_org_with_default__icontains=search_term)).values('id','display_name')
...
class in_network_with_api(DeveloperErrorViewMixin, APIView):
    ...
    def get(self, request, **kwargs):
        ...
        associations_list = Organization.objects.filter(active=1,homepage_in_network_with=1)
        association_list = []
...
class topic_course_lect_count_api(DeveloperErrorViewMixin, APIView):
    ...
    def get(self, request, **kwargs):
        ...
        cat_course_count = course_extrainfo.objects.exclude(category='').values('category').annotate(ccount=Count('category')).filter(ccount__gte=1).order_by('-ccount')[:8]
        categoryid = []
        ...
        if categories_list:
            for category in  categories_list:
                ...
                category_dict['topic_url'] = "https://learn.docmode.org/subjects/"+category['topic_short_name']
                if courses_count:
                ...
                if case_studies_count:
                    category_dict['case_studies_count'] = case_studies_count[0]['ccount']
...
class partners_api(DeveloperErrorViewMixin, APIView):
    ...
    def get(self, request, **kwargs):
        user = request.user
        associations_list = Organization.objects.filter(marketing_display=1).order_by('name')
        ...
        if association:
            ...
            if user.is_authenticated():
                if user.is_staff:
                    ...
                    association_dict['dashboard_url'] = 'https://learn.docmode.org/dashboard/'+association.short_name+'/'
                else:
                    try:
                        assoc_member_admin = OrganizationMembers.objects.get(organization_id=association.id,is_admin=1,user_id=user.id)
                        association_dict['is_staff'] = 'staff'
                        association_dict['dashboard_url'] = 'https://learn.docmode.org/dashboard/'+association.short_name+'/'
                    except ObjectDoesNotExist:
            ...
            courses = CourseOverview.objects.all().filter(display_org_with_default=org,catalog_visibility="both").order_by('start')[::-1]
            assoc_course_list = []
            for course in courses:
                assoc_course_dict = {}
                ...
                course_name = (course.display_name[:40] + '..') if len(course.display_name) > 50 else course.display_name
                ...
                if course_extra_info:
                    assoc_course_dict['wp_url'] = "https://docmode.org/"+course_extra_info.course_seo_url
            ...
            association_list.append(association_dict)
            return JsonResponse(association_list, status=status.HTTP_400_BAD_REQUEST, safe=False)
...
class wp_partner_admin_api(DeveloperErrorViewMixin, APIView):
    ...
    def get(self, request, **kwargs):
        ...
        try:
            assoc_member_admin = OrganizationMembers.objects.get(organization_id=assoc,is_admin=1,user_id=user_details)
            is_admin = 'Yes'
        except ObjectDoesNotExist:
            is_admin = 'No'
...
def course_detail(request, username, course_key):
    ...
    association = Organization.objects.get(short_name=course_org.display_org_with_default,active=1)
    courses = CourseOverview.objects.all().filter(display_org_with_default=course_org.org,catalog_visibility="both").order_by('start')[::-1]
...
class wp_home_ongoing_courses(DeveloperErrorViewMixin, RetrieveAPIView):
    ...
    def get(self, request, **kwargs):
        ...
        courses = CourseOverview.objects.all().filter(pk__in=index_cid,start__lte=today_date_time,end__gte=today_date_time,catalog_visibility='both').order_by('start')[::-1]
        ongoing_course_list = []
        for course in courses:
            ongoing_course_dict = {}
            course_id = str(course.id)
            course_extra_info = course_extrainfo.objects.get(course_id=course.id)
            #log.info(u"query %s",course.id)
            assoc_name = Organization.objects.get(short_name=course.display_org_with_default)
            ...
            if course_extra_info:
                ongoing_course_dict['wp_url'] = "https://docmode.org/"+course_extra_info.course_seo_url
...
class wp_home_upcoming_lectures(DeveloperErrorViewMixin, RetrieveAPIView):
    ...
    def get(self, request, **kwargs):
        ...
        courses = CourseOverview.objects.all().filter(pk__in=index_cid,start__gte=today_date_time,catalog_visibility='both')
        ongoing_course_list = []
        for course in courses:
            ongoing_course_dict = {}
            course_id = str(course.id)
...
class user_profile_api(DeveloperErrorViewMixin, APIView):
    ...
    def get(self, request, **kwargs):
        ...
        user_enrolled_courses = CourseEnrollment.objects.filter(user_id=user.id,is_active=1)
        cid = []
        ...
        courses = CourseOverview.objects.all().filter(pk__in=cid).order_by('start')[::-1]
        user_enroll_list = []
        assoc_course_list = []
        for course in courses:
            ...
            course_extra_info = course_extrainfo.objects.get(course_id=course.id)
            #log.info(u"query %s",course.id)
            association = Organization.objects.get(short_name=course.display_org_with_default)
            data = association.name 
            info = (data[:8] + '..') if len(data) > 10 else data
            course_name = (course.display_name[:40] + '..') if len(course.display_name) > 50 else course.display_name
            assoc_course_dict['course_title'] = course_name
            ...
            assoc_course_dict["media"] = course.course_image_url
            if course_extra_info:
                assoc_course_dict['wp_url'] = "https://www.docmode.org/"+course_extra_info.course_seo_url
        ...
        clinic_hospital_data = clinic_hospital_address.objects.all().filter(user=user.id)
        ...
        for certificate in course_certificates:                   
            ...
            cert_org = Organization.objects.get(short_name=courseid.display_org_with_default)
            course_name = (courseid.display_name_with_default[:55]) if len(courseid.display_name_with_default) > 40 else courseid.display_name_with_default
            ...
            if ctype_numb == '1':
                ...
                user_certificate_dict['medallion'] = 'https://s3-ap-southeast-1.amazonaws.com/docmode.co/website-images/Medal-Courses.png'
            elif ctype_numb == '2':
                user_certificate_dict['completed_title'] = 'Lecture Completed'
                user_certificate_dict['medallion'] = 'https://s3-ap-southeast-1.amazonaws.com/docmode.co/website-images/Medal-Lectures.png'
...
class ad_manager_api(DeveloperErrorViewMixin, APIView):
    ...
    def get(self, request, **kwargs):
        from lms.djangoapps.add_manager.views import sponsored_user,get_association_ad_data,user_ad_view_counter
        from lms.djangoapps.reg_form.views import get_authuser
        ...
        if 'course_id' and 'user' in request.GET:
            ...
            if user_course != 'n/a':
                ...
                if user_ad_view > 0 :
                    ...
                    update_counter = user_view_counter.objects.filter(course_id=course_id,user=user.id).update(mcounter = F('mcounter')+1)
                else:
                    result['first_time'] = "true"
                    countr = user_view_counter(user_id=user.id, course_id=course_id,mcounter=1)
                    countr.save()
            else:
                result = {}
...
class wp_courses(DeveloperErrorViewMixin, RetrieveAPIView):
    ...
    def get(self, request, **kwargs):
        ...
        today_date_time = datetime.datetime.now(UTC)
        courses = CourseOverview.objects.all().filter(pk__in=index_cid,catalog_visibility='both').order_by('start')[::-1]
        ongoing_course_list = []
        for course in courses:
            ongoing_course_dict = {}
            course_id = str(course.id)
        ...
        if request.method == 'GET':
            ...
            # except PermissionDenied:
                for key, value in request.GET.iteritems():
                    userdata = {}
                    userdata[key] = value
                registration_log = third_party_user_registration_log(email=email,status='Account creation not allowed either the user is already registered or email-id not valid',data=userdata)
            ...
            profile.save()
            user_extrainfo = extrafields(phone=phone,rcountry=country,rstate=state,rcity=city,rpincode=pincode,user_type=user_type,user=user)
            user_extrainfo.save()
            ...
            for key, value in request.GET.iteritems():
                userdata = {}
                userdata[key] = value
            registration_log = third_party_user_registration_log(email=email,status='succesful',data=userdata)
            registration_log.save()
            home_ongoing_course_list = []
            home_ongoing_course_dict = {}
            ...
            for key, value in request.GET.iteritems():
                userdata = {}
                userdata[key] = value
            registration_log = third_party_user_registration_log(email=email,status='failed',data=userdata)
            registration_log.save()
