...
def cs_addnew(request):
    if request.method == "POST":
        ...
        author_affiliation = ",".join(
            map(str, request.POST.getlist("author_affiliations[]"))
        )
        tos = request.POST.get("tos", "")
        case_study = case_study_abstracts(
            user_id=user,
            title=title,
        ...
        context = {"errors": "", "csrf": csrf(request)["csrf_token"]}
        return render_to_response("case_study/case_study_add.html", context)
...
def cs_update(request, caseid):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            context = {"case_data": case_dat2, "errors": msg}
...
def csy_about(request):
    ...
    context = {"cs_competition": abstract}
