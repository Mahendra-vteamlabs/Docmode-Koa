...
def _activate_organization(organization):
    ...
    """
    [
        _activate_organization_course_relationship(record)
        for record in internal.OrganizationCourse.objects.filter(
            organization_id=organization.id, active=False
        )
    ]

    [
        _activate_record(record)
        for record in internal.Organization.objects.filter(
            id=organization.id, active=False
        )
    ]
...
def _inactivate_organization(organization):
    ...
    """
    [
        _inactivate_organization_course_relationship(record)
        for record in internal.OrganizationCourse.objects.filter(
    ...
    [
        ...
        for record in internal.Organization.objects.filter(
            id=organization.id, active=True
        )
    ]


def _activate_organization_course_relationship(
    relationship,
    ...
    relationship = internal.OrganizationCourse.objects.get(
        id=relationship.id, active=False, organization__active=True
    )
...
def _inactivate_organization_course_relationship(
    ...
    """
    relationship = internal.OrganizationCourse.objects.get(
        id=relationship.id, active=True
    )
...
def create_organization(organization):
    ...
    if not organization.get("name"):
        exceptions.raise_exception(
            "organization", organization, exceptions.InvalidOrganizationException
        )
...
def fetch_organizations():
    ...
    return serializers.serialize_organizations(
        internal.Organization.objects.filter(active=True)
    )
...
def fetch_organization_courses(organization):
    ...
    queryset = internal.OrganizationCourse.objects.filter(
        organization=organization_obj, active=True
