...
from common.djangoapps.organizations.tests.factories import (
    UserFactory,
    OrganizationFactory,
)
...
class TestOrganizationsView(TestCase):
    ...
    def _get_organization_url(self, organization):
        ...
        return reverse(
            "v0:organization-detail", kwargs={"short_name": organization.short_name}
        )
    ...
    def test_single_organization(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data, OrganizationSerializer(self.organization).data
